...
// auto run
func init() {
	server.RegisterInit(initOrderDict, &initDict{})
}
...
func (i *initDict) MigrateTable(ctx context.Context) (context.Context, error) {
	...
	return ctx, db.AutoMigrate(&system.SysDictionary{})
}
...
func (i *initDict) TableCreated(ctx context.Context) bool {
	...
	return db.Migrator().HasTable(&system.SysDictionary{})
}
...
func (i *initDict) DataInserted(ctx context.Context) bool {
	...
	if errors.Is(db.Where("type = ?", "bool").First(&system.SysDictionary{}).Error, gorm.ErrRecordNotFound) { // 判断是否存在数据
		return false
	}
	return true
}
