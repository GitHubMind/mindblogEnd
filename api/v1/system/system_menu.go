...
// @Param data body request.EmptyJwt true "空"
// @Success 200 {object} response.Response{data=response.SysMenusResponse,msg=string} "获取用户动态路由,返回包括系统菜单详情列表"
// @Router /menu/getMenu [post]
func (receiver AuthorityMenuApi) GetMenu(c *gin.Context) {
	if menus, err := menuService.GetMenuTree(lib.GetUserAuthorityId(c)); err != nil {
...
func (a *AuthorityMenuApi) AddMenuAuthority(c *gin.Context) {
	...
	if err := lib.Verify(authorityMenu, lib.AuthorityIdVerify); err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}
	if err := menuService.AddMenuAuthority(authorityMenu.Menus, authorityMenu.AuthorityId); err != nil {
		global.GM_LOG.Error("添加失败!", zap.Error(err))
...
func (a *AuthorityMenuApi) UpdateBaseMenu(c *gin.Context) {
	...
	if err := lib.Verify(menu, lib.MenuVerify); err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}
	if err := lib.Verify(menu.Meta, lib.MenuMetaVerify); err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}
...
func (a *AuthorityMenuApi) DeleteBaseMenu(c *gin.Context) {
	...
	_ = c.ShouldBindJSON(&menu)
	if err := lib.Verify(menu, lib.IdVerify); err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}
